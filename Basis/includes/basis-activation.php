...
if (is_admin() && 'themes.php' === $pagenow && isset( $_GET['activated'])) {
  ...
  foreach ($pages_to_create as $new_page_title) {
	...
	$basis_nav_theme_mod = false;

	if (!has_nav_menu('primary_navigation')) {
		$primary_nav_id = wp_create_nav_menu('Primary Navigation', array('slug' => 'primary_navigation'));
		$basis_nav_theme_mod['primary_navigation'] = $primary_nav_id;
	}	

	if (!has_nav_menu('utility_navigation')) {
		$utility_nav_id = wp_create_nav_menu('Utility Navigation', array('slug' => 'utility_navigation'));
		$basis_nav_theme_mod['utility_navigation'] = $utility_nav_id;
	}

	if ($basis_nav_theme_mod) { 
    set_theme_mod('nav_menu_locations', $basis_nav_theme_mod );
  }
